...
欢迎一切 能够实现任何相关数据解析算法/乐意适配其他平台者 参与本项目以及 [QQ-History-Backup](https://github.com/QQBackup/QQ-History-Backup/tree/dev) 的开发！[邮箱](mailto:ly-niko@qq.com)，或者直接开 PR/issue。文档的写作风格随意（但建议认真编写图片的 替代文本 和 文件名），也可以只加入一个指向你的 仓库/博客 等的链接。
...
本项目不对生成内容的完整性、准确性作任何担保，生成的一切内容**不可用于法律取证**，你不应当将其用于学习与交流外的任何用途。

本项目遵循`LICENSE`里的开源协议，大致要求**标识项目地址**且**禁止商用**；部分文件同时以不同的协议发布，具体参见文件内对应的声明。
